...
function getPolynom(...param) {
  ...
  if (param.length === 2) {
    return (y) => param[0] * y + param[1];
  }
  if (param.length === 3) {
    return (y) => param[0] * y ** 2 + param[1] * y + param[2];
  }
  return null;
}
...
function logger(func, logFunc) {
  ...
  return function () {
    ...
    const functionArguments = args.map((arg) => JSON.stringify(arg)).join(',');
    const functionName = func.name || 'anonymous';
    logFunc(`${functionName}(${functionArguments}) starts`);
    const result = func.apply(this, args);
    logFunc(`${functionName}(${functionArguments}) ends`);
    return result;
  };
}
